...
class UIPrediction:
    ...
    def __init__(self):
        ...
        if self.lm is None:
            ...
            try:
                self.lm = pickle.load(open(modelname, 'rb'))
    ...
    def create_ui(self):
        ...
        # scrollbor.grid(row=0, column=1, sticky="ns")
        ...
        rmse_label.grid(row=2, column=8)
        ...
        # --- Load Model ---
        loadmodel_button = Button(loadmodel_panel, text="6. Load Model", command=self.load_model)
        loadmodel_button.grid(row=0, column=0)

        # danh sách model có sẵn
        current_dir = os.path.dirname(os.path.abspath(__file__))
        self.model_options = [
            os.path.basename(f) for f in glob.glob(os.path.join(current_dir, "housingmodel_*.zip"))
        ]

        if self.model_options:
            default_model = self.model_options[0]
        else:
        ...
        self.model_option_menu = OptionMenu(loadmodel_panel, self.selected_model, *self.model_options)
        self.model_option_menu.grid(row=0, column=1)
    ...
    def do_train(self):
        ...
        self.X = self.df[['Avg. Area Income', 'Avg. Area House Age', 'Avg. Area Number of Rooms','Avg. Area Number of Bedrooms', 'Area Population' ]]
        self.y = self.df['Price']

        self.X_train, self.X_test, self.y_train, self.y_test = train_test_split(self.X, self.y, test_size=1 - ratio, random_state=101)
    ...
    def do_evaluation(self):
        ...
        for i in range(len(self.X_test)):
            values = [
                ...
                self.y_test.iloc[i],  #dùng iloc thay vì self.y_test[i]
                predictions[i]
            ]
    ...
    def do_save_model(self):
        if self.lm is None:
            messagebox.showwarning("Warning", "Chưa có mô hình nào để lưu!")
            return

        confirm = messagebox.askyesno("Xác nhận", "Bạn có chắc muốn lưu mô hình hiện tại?")
        if not confirm:
            return

        try:
            # Tạo tên model có timestamp
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            model_name = f"housingmodel_{timestamp}.zip"
            ...
            messagebox.showinfo("Thành công", f"Đã lưu mô hình: {model_name}")
            print(f"Model saved successfully at {model_path}")

            # Cập nhật danh sách model trong OptionMenu
            self.update_model_list()

        except Exception as e:
            messagebox.showerror("Lỗi", f"Không thể lưu model:\n{e}")
    ...
    def load_model(self):
        ...
        if model_name == "(Chưa có model nào)":
            messagebox.showwarning("Cảnh báo", "Chưa có mô hình nào để nạp.")
            return
    ...
    def do_prediction(self):
        input_data = np.array([[self.area_income_value.get(),
    ...
    def update_model_list(self):
        ...
        for model in self.model_options:
            menu.add_command(label=model, command=lambda value=model: self.selected_model.set(value))
